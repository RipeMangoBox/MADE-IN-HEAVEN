...
from modules import GatedPixelCNN, Config
import numpy as np
...
MNIST_Config = Config(
    IMAGE_SHAPE = (28, 28),
    INPUT_DIM = 1,  # 1 (Grayscale)
)

FashionMNIST_Config = Config(
    IMAGE_SHAPE = (28, 28),  # (28, 28)
    INPUT_DIM = 3,  # 3 (RGB)
)
...
config_dict = {'MNIST': MNIST_Config, 'FashionMNIST': FashionMNIST_Config, 'CIFAR10': CIFAR10_Config}
...
def generate_samples(label=None, dataset=None):
    if label is None:
    ...
    if dataset is not None:
        ...
        global IMAGE_SHAPE, DATASET, INPUT_DIM
        DATASET = dataset
        IMAGE_SHAPE, INPUT_DIM = vars(config).values()
